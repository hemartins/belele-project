...
public class TeamController
{
	...
	public Double getTeamQuality(Season season)
	{
		...
		if (s != null)
		{
			return s.getPoints().doubleValue() / s.getPlayedGames().doubleValue();
		} else
			return null;
	...
	{
		return Days.daysBetween(getBeforeFixture(Integer.valueOf(nextFixture.getSeason().getYear()), nextFixture.getDate(), null).getDate(),
				nextFixture.getDate()).getDays();
	}

	// Rating dos ultimos jogos, venue opcional, ratings ordenados por ordem
	// crescente
	public Double getLastFixturesRating(Fixture nextFixture, Venue venue, Integer numberOfFixtures, List<Double> ratings, ResultType type)
	{
		...
		List<Fixture> fixtures = getBeforeFixtures(nextFixture.getSeason(), nextFixture.getMatchday(), venue, numberOfFixtures);
	...
	{
		List<Fixture> fixtures = getBeforeFixtures(nextFixture.getSeason(), nextFixture.getMatchday(), venue, nextFixture.getMatchday());
	...
	// Rating do h2h
	public Double getH2HRating(Fixture nextFixture, List<Double> ratings, ResultType type)
	{
		double rating = 0;
	...
	public Fixture getBeforeFixture(Integer year, DateTime date, Venue venue)
	{
		...
		for (Fixture f : team.getFixtures(year, venue))
		{
			...
			{
				if (f.getDate().isAfter(fixture.getDate()) && f.getDate().isBefore(date))
					fixture = f;
			}
		}
		return fixture;
	...
	public List<Fixture> getBeforeFixtures(Season season, Integer matchday, Venue venue, Integer numberOfFixtures)
	{
		List<Fixture> fixtures = new ArrayList<Fixture>();
		for (Fixture f : team.getFixtures(Integer.valueOf(season.getYear()), venue))
		{
			if (f.getMatchday() < matchday && f.getSeason().getCaption().equals(season.getCaption()))
				fixtures.add(f);
		}
		return fixtures.subList(fixtures.size() - 1 - numberOfFixtures > 0 ? fixtures.size() - 1 - numberOfFixtures : 0, fixtures.size() - 1);
	}
