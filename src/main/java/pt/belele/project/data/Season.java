...
public class Season {
	...
	private List<Team> getTeamsResource() {
		TeamsResource teams = (TeamsResource) Database.getObject(
				season.getTeams(), TeamsResource.class);
	...
	private List<Fixture> getFixturesResource() {
		SeasonFixtureResource fixtures = (SeasonFixtureResource) Database
				.getObject(season.getFixtures(), SeasonFixtureResource.class);
	...
	private LeagueTable getLeagueTableResource() {
		return new LeagueTable((LeagueTableResource) Database.getObject(
				season.getLeagueTable(), LeagueTableResource.class));
	}
	...
	private LeagueTable getLeagueTableResource(int matchday) {
		...
		params.add("matchday", matchday);
		return new LeagueTable((LeagueTableResource) Database.getObject(
				season.getLeagueTable(), LeagueTableResource.class, params));
	}

	private SeasonsResource getSeasons(int year) {
		MultivaluedMap<String, Object> params = new MultivaluedHashMap<String, Object>();
		params.add("season", year);
		SeasonResource[] arr = (SeasonResource[]) Database.getObject(
				API_ENDPOINT, SeasonResource[].class, params);
